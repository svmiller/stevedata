...
- `uid`:
  ...
  a numeric vector, a unique identifier for the respondent as they first
  appear in the CCES data.
...
- `pid7na`:
  ...
  a numeric vector for the respondent's partisanship on the familiar 1-7
  scale. 1 = Strong Democrat. 7 = Strong Republican. Other party
...
- `lrelig`:
  ...
  a numeric vector that serves as a latent estimate for religiosity from
  the `bornagain`, `religimp`, `churchatd`, and `prayerfreq` variables.
  Higher values = more religiosity.

- `lcograc`:

  a numeric vector that serves as a latent estimate for cognitive
  racism. This is derived from the `racerare` and `whiteadv` variables.

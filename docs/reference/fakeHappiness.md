...
- `e`:
  ...
  random noise, generated from a normal distribution with a mean of 0
...
- `z_happy`:
  ...
  the same arbitrary happiness variable, scaled to have a mean of 0 and
  a standard deviation of 1. This makes it seem more "latent".

## Details

Data are randomly sampled from the `TV16` data set in the same package
for the age, female, college education, family income, and born-again
variables. Thereafter, I created an arbitrary "happiness" variable that
is equal to
`100 - .95*age + .01*(age^2) + .25*female + .05*famincr + .1*bornagain + e`.
The data are not supposed to be realistic, per se. They're supposed to

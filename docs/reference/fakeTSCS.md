...
There are 100 observations for 25 hypothetical countries. The outcome
`y` is a linear function of a baseline for each hypothetical country,
plus a yearly growth trend as well as varying growth errors for each
country. `x1` is supposed to have a linear effect of .5 on `y`, all
things considered. `x2` is supposed to have a linear effect of 1 on `y`
for each unit change in `x2`, all things considered.

...
- `category`:
  ...
  a character vector for whether the `count` is inbound to the U.S. from
  the `area` variable or outbound (i.e. American expats) to the `area`
  variable in a given year.

- `area`:

  a character vector for the area of origin (if `category` == "Inbound")
  or destination for American migrants (if `category` == "Outbound")

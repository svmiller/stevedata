...
- `pop`:
  ...
  a numeric vector for residents (per thousand) under 18 years of age in
  1974

- `edexppc`:

  a numeric vector for per capita public school expenditures in a state,
  projected for 1975.

## Details

I copied these data from the `robustbase` package. I just didn't want to
make my students install it. Note: I'm pretty sure "NB" was suppose to
be "NE" and that "DY" is supposed to be "KY". I made those changes.

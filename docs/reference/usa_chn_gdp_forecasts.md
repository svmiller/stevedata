...
from 2014, and forecasts from the `forecast` and `prophet` packages in
R.
...
`prophet` forecasts have a prefix of `p_`. Forecasts are not meant to be
exhaustive (clearly), only illustrative for in-class discussion about
the "Rise of China." Forecasts made in R on Nov. 20, 2020.
